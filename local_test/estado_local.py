...
class EstadoLocal:
    ...
    def _expirar_micro_se_necessario(self):
        if not self.dados.get("micro_ativo"):
            return
        expira_em = self.dados.get("micro_expira_em")
        if not expira_em:
            return
        try:
        ...
        if limite is not None and agora_local() >= limite:
            self.desativar_micro()

    def obter_estado(self, estado_vazio, veiculo="principal"):
        if veiculo == "micro":
            self._expirar_micro_se_necessario()
    ...
    def salvar_estado(self, estado, veiculo="principal"):
        chave = "estado_micro" if veiculo == "micro" else "estado"
        self.dados[chave] = estado
        self._salvar()

    def micro_esta_ativo(self):
        self._expirar_micro_se_necessario()
        return bool(self.dados.get("micro_ativo"))

    def ativar_micro(self):
        if self.micro_esta_ativo():
            return {"ok": True, "ja_ativo": True}

        agora = agora_local()
        self.dados["micro_ativo"] = True
        self.dados["micro_ativado_em"] = agora.isoformat()

        inicio_oficial = agora.replace(hour=7, minute=25, second=0, microsecond=0)
        fim_carencia = agora.replace(hour=13, minute=0, second=0, microsecond=0)
        if inicio_oficial <= agora < fim_carencia:
            self.dados["micro_expira_em"] = fim_carencia.isoformat()
        else:
            self.dados["micro_expira_em"] = None
    ...
    def desativar_micro(self):
        self.dados["micro_ativo"] = False
        self.dados["micro_ativado_em"] = None
        self.dados["micro_expira_em"] = None
        self.dados["estado_micro"] = None
        self._salvar()
    ...
    def adicionar_aviso(self, texto):
        ...
        if not self.dados.get("avisos_expiram_em"):
            self.dados["avisos_expiram_em"] = expiracao_bloco_aviso(agora_local()).isoformat()
        self._salvar()
        return {"ok":True,"duplicado":False,"avisos":avisos,"expiram_em":self.dados.get("avisos_expiram_em")}
    ...
    def limpar_estado(self, estado_vazio):
        ...
        self.dados["estado_micro"] = estado_vazio()
        self._salvar()
