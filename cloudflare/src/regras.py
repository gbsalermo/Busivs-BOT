...
# Garagem faz parte apenas do encerramento especial da última volta do bloco.
# Ela entra no catálogo compartilhado para aparecer no teclado, mas o backend
# rejeita a confirmação fora do contexto de retorno à garagem.
...
def _bloco_ultima_volta_atual(agora):
    """Retorna o bloco cuja última volta já saiu e ainda não começou o próximo."""
    candidatos = []
    for indice, bloco in enumerate(BLOCOS_PRINCIPAL):
        ...
        candidatos.append({
            **bloco,
            "ultima_dt": ultima_dt,
            "proximo_inicio_dt": proximo_inicio,
        })
